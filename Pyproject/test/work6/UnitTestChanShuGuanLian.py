...
class UnitTestChanShuGuanLian(unittest.TestCase):
    def setUp(self):
        self.Headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.84 Safari/537.36'}
        self.AddDailyLogURL = 'https://i.cnblogs.com/EditDiary.aspx?opt=1'
        self.AddData = {'__VIEWSTATE': '', '__VIEWSTATEGENERATOR': '4773056F', 'Editor$Edit$txbTitle': '测试334标题',
        ...
        self.AddReq = self.session.post(self.AddDailyLogURL, data=self.AddData, headers=self.Headers, verify=True,
    ...
    def tearDown(self):
        ...
        print(DelData)
        DelReq = self.session.post(self.DelDailyLogURL, json=DelData, headers=self.Headers, verify=True,
                                   allow_redirects=True)
